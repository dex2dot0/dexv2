<script lang="ts">
	export let isSelected: boolean = false;
</script>

<svg
   class={isSelected ? "rounded-full bg-gradient-to-r from-surface-500 fill-secondary-500 stroke-secondary-800 w-12 h-16" : "fill-surface-200/30 stroke-surface-500/30 w-8 h-8"}
   width="120"
   height="60"
   viewBox="0 0 120 60">
  <defs
     id="defs3478" />
  <sodipodi:namedview
     id="base"
     borderopacity="1.0"
     inkscape:pageopacity="0.0"
     inkscape:pageshadow="2"
     inkscape:zoom="6.0991803"
     inkscape:cx="37.811232"
     inkscape:cy="51.471985"
     inkscape:document-units="px"
     inkscape:current-layer="layer1"
     showgrid="false"
     fit-margin-top="0"
     fit-margin-left="0"
     fit-margin-right="0"
     fit-margin-bottom="0"
     inkscape:window-width="1850"
     inkscape:window-height="1016"
     inkscape:window-x="70"
     inkscape:window-y="27"
     inkscape:window-maximized="1"
     units="px" />
  <metadata
     id="metadata3481">
    <rdf:RDF>
      <cc:Work
         rdf:about="">
        <dc:format>image/svg+xml</dc:format>
        <dc:type
           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />
        <dc:title></dc:title>
      </cc:Work>
    </rdf:RDF>
  </metadata>
  <g
     id="layer1"
     transform="translate(-193.633,-732.11218)">
    <g
       id="g29"
       transform="translate(-10.821126,3.1151727)">
      <path
         style="stroke-width:0.31431133"
         id="path2999"
         d="m 257.17003,747.19755 h 17.28712 v 7.22917 h -10.05796 v 7.22916 h 10.05796 v 7.22916 h -17.28712 z"
         inkscape:connector-curvature="0" />
      <path
         style="stroke-width:0.31431133"
         id="path3001"
         d="m 277.91457,747.19755 h 17.28713 v 6.28624 h -10.05796 v 1.25723 h 10.05796 v 14.45833 h -17.28713 v -6.60054 h 10.05796 v -1.25724 h -10.05796 z"
         inkscape:connector-curvature="0" />
      <path
         style="stroke-width:0.31431133"
         id="path3003"
         d="m 298.65913,747.19755 h 17.28711 v 6.28624 h -10.05796 v 1.25723 h 10.05796 v 14.45833 h -17.28711 v -6.60054 h 10.05797 v -1.25724 h -10.05797 z"
         inkscape:connector-curvature="0" />
    </g>
    <g
       id="g37"
       transform="matrix(0.60400616,0,0,0.60400616,83.235735,307.26789)">
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon2989"
         points="404.321,470.819 255.778,512 107.644,470.877 74.633,100.62 437.367,100.62 " />
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon2991"
         points="256,480.523 376.03,447.246 404.27,130.894 256,130.894 " />
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon2993"
         points="256,268.217 150.31,268.217 154.38,313.627 256,313.627 " />
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon2995"
         points="255.843,176.305 142.132,176.305 146.26,221.716 256,221.716 256,176.305 " />
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon2997"
         points="255.801,386.206 205.227,372.55 201.994,336.333 177.419,336.333 156.409,336.333 162.771,407.634 255.791,433.457 256,433.399 256,386.153 " />
      <polygon
         transform="matrix(0.17578125,0,0,0.17578125,180.51392,698.3622)"
         id="polygon3005"
         points="319.831,221.716 315.699,268.217 255.843,268.217 255.843,313.627 311.761,313.627 306.49,372.521 255.843,386.191 255.843,433.435 348.937,407.634 349.62,399.962 360.291,280.411 361.399,268.217 369.597,176.305 255.843,176.305 255.843,221.716 " />
    </g>
  </g>
</svg>
